...
export default function NewLayout() {
  ...
  const getStatusIcon = (s) => {
    if (s.includes('On Track')) return <CheckCircle className="w-3.5 h-3.5" />
    if (s.includes('Critical')) return <AlertCircle className="w-3.5 h-3.5" />
      ...
      <div className="h-[calc(100vh-82px)] flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="px-5 pt-3 pb-2.5 flex items-start justify-between gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 mb-1.5 text-[10px]">
                <Link to="/" className="text-ink-muted hover:text-navy-800 transition font-medium flex items-center gap-1">
                  <ArrowLeft className="w-3 h-3" />
                  Dashboard
                </Link>
                <span className="text-surface-border">/</span>
                <span className="font-bold text-navy-800 bg-surface-2 px-1.5 py-0.5 rounded font-mono">{goal.mgCode}</span>
                <span className="text-surface-border">/</span>
                <span className="font-medium text-ink-muted">{goal.pillar}</span>
              </div>

              {/* Goal title with switcher */}
              <div className="flex items-center gap-2">
                <h1 className="font-display text-lg font-bold text-navy-800 leading-snug">
                  {goal.macroGoal}
                </h1>
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-1">
                  ...
                  <span className="text-[9px] font-mono-num text-ink-muted font-bold">{goalIndex + 1}/{macroGoals.length}</span>
                  <button
              ...
              <div className="flex items-center gap-2 mt-1 text-[10px] text-ink-muted">
                ...
                <Bookmark className="w-3 h-3 text-navy-400" />
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <div className="flex bg-surface-2/60 rounded-xl p-0.5 border border-surface-border/50">
                {['2030', '2047'].map(h => (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-lg text-[11px] font-bold transition cursor-pointer ${
                      horizon === h 
              ...
              {/* Status pill */}
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border font-bold text-xs ring-2 ${accent.ring} ${statusMeta.bg} ${statusMeta.text} ${statusMeta.border}`}>
        ...
        <div className="flex-1 min-h-0 px-5 pb-3 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-12 gap-3 flex-1 min-h-0">
            ...
            <div className="col-span-3 flex flex-col gap-2.5 min-h-0">
              ...
              <div className="bg-white border border-slate-100 rounded-xl p-3.5 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-[9px] text-slate-400 font-medium mt-1">Reference starting point</p>
              </div>

              {/* Target 2030 */}
              <div className="bg-white border border-emerald-100 rounded-xl p-3.5 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[9px] font-bold text-emerald-400 uppercase tracking-widest">Target 2030</span>
                  <div className="w-7 h-7 rounded-lg bg-emerald-50 flex items-center justify-center">
                    <Target className="w-3.5 h-3.5 text-emerald-500" />
                  </div>
                </div>
                <p className="text-2xl font-bold font-mono-num text-navy-800 leading-none">{goal.target2030}</p>
                <p className="text-[9px] text-emerald-500/70 font-medium mt-1">
                  Gap: {goal.gap2030Ratio !== null ? `${(goal.gap2030Ratio * 100).toFixed(0)}%` : 'N/A'}
                </p>
              </div>

              {/* Target 2047 */}
              <div className="bg-white border border-violet-100 rounded-xl p-3.5 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[9px] font-bold text-violet-400 uppercase tracking-widest">Target 2047</span>
                  <div className="w-7 h-7 rounded-lg bg-violet-50 flex items-center justify-center">
                    <Zap className="w-3.5 h-3.5 text-violet-500" />
                  </div>
                </div>
                <p className="text-2xl font-bold font-mono-num text-navy-800 leading-none">{goal.target2047}</p>
                <p className="text-[9px] text-violet-500/70 font-medium mt-1">
                  Gap: {goal.gap2047Ratio !== null ? `${(goal.gap2047Ratio * 100).toFixed(0)}%` : 'N/A'}
                </p>
              </div>

              {/* Key Initiatives — fills remaining space */}
              <div className="bg-white border border-saffron-100 rounded-xl p-3.5 shadow-sm flex-1 min-h-0 flex flex-col overflow-hidden">
                <div className="flex items-center gap-1.5 mb-2 flex-shrink-0">
                  <Sparkles className="w-3.5 h-3.5 text-saffron-500" />
                  <h4 className="text-[9px] font-bold text-navy-800 uppercase tracking-widest">Key Initiatives</h4>
                </div>
                <div className="flex-1 overflow-y-auto min-h-0 space-y-2 pr-0.5">
                  {initiatives.map((ini, idx) => (
                    <div key={idx} className="flex items-start gap-2">
                      <span className="w-5 h-5 rounded-lg bg-gradient-to-br from-saffron-400 to-orange-400 text-white text-[8px] font-bold flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm">
                        {idx + 1}
                      </span>
                      <p className="text-[10px] text-ink-body leading-snug font-medium">{ini}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex-shrink-0 bg-white border border-surface-border rounded-xl shadow-sm overflow-hidden">
            <div className="px-4 py-2.5 border-b border-surface-2/50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Activity className="w-4 h-4 text-navy-500" />
                <h3 className="text-[11px] font-bold text-navy-800 uppercase tracking-wider">CAGR Performance Analysis</h3>
              </div>
              <span className="text-[9px] font-medium text-ink-muted bg-surface-0 border border-surface-border px-2.5 py-0.5 rounded-full uppercase tracking-wider">
            ...
            {!goal.hasCagrAnalysis ? (
              ...
              <div className="grid grid-cols-2 divide-x divide-surface-2/50">
                {/* 2030 */}
                <CagrCard 
                  label="2030 Horizon"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={() => setIsChartExpanded(false)}>
          ...
          >
            ...
            <div className="flex-1 min-h-0 p-5">
              <GoalTrendChart goal={goal} trend={trend} horizon={horizon} onHorizonChange={setHorizon} isExpanded={true} />
            </div>
          </div>
        </div>
      )}
...
function CagrCard({ label, dotColor, target, currentCagr, requiredCagr, achievement, status, isActive }) {
  ...
  return (
    <div className={`px-5 py-3.5 transition ${isActive ? 'bg-navy-50/30' : ''}`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className={`w-2.5 h-2.5 rounded-full ${dotColor}`} />
          <span className="text-xs font-bold text-navy-800">{label}</span>
        </div>
        <span className={`inline-flex items-center px-2.5 py-0.5 text-[9px] font-bold rounded-lg border ${style.bg} ${style.text} ${style.border}`}>
          {status.split(' (')[0]}
        </span>
      </div>
      <div className="grid grid-cols-4 gap-4">
        <div>
          <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Target</p>
          <p className="text-base font-bold font-mono-num text-navy-800">{target}</p>
        </div>
        <div>
          <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Current CAGR</p>
          <p className="text-base font-bold font-mono-num text-blue-600">
            {currentCagr !== null ? `${(currentCagr * 100).toFixed(2)}%` : 'N/A'}
          </p>
        </div>
        <div>
          <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Required CAGR</p>
          <p className="text-base font-bold font-mono-num text-saffron-600">
            {requiredCagr !== null ? `${(requiredCagr * 100).toFixed(2)}%` : 'N/A'}
          </p>
        </div>
        <div>
          <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Achievement</p>
          <p className="text-base font-bold font-mono-num text-navy-800">{achievement || 'N/A'}</p>
        </div>
      </div>
    </div>
  )
